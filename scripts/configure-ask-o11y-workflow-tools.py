...
- Use isolated Python only when generated computation is needed. Generate only the Python required by the confirmed request. The sandbox receives `df`, `pd`, `np`, `display(value)`, and `emit(value, name=None)`; it has no datasource credentials or network. When an analysis dashboard is requested, emit a Matplotlib PNG plus optional text summary; never use Sandbox output as native Grafana chart data. Never ask the sandbox to query Grafana, install packages, read host files, or recover secrets.
