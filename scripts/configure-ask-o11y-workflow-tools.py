...
- For the authorized `wferp` dataset only, preserve the old LLM-first SQL mode through the current trust seams: call `data-query-planner_search_wferp_schema` with the exact user request to obtain a bounded multilingual schema context, author exactly one legacy-compatible MSSQL SELECT yourself using only that context, then call `data-query-planner_plan_wferp_query` with the exact original request, SQL, and projected output field names. On a recoverable validation error, revise only the SQL using the returned repair code/hint and retry at most two times. The LLM authors WFERP SQL; Data Query Planner validates and stores it but never executes it. Never use this tool for other datasets, never submit raw SQL to Grafana Query or built-in Grafana tools, and never bypass the user-visible Analysis Preview. Show the accepted SQL and validated tables in that Preview before confirmation.
