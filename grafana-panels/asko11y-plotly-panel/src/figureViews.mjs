...
export function splitFigureViews(figure, selectedViewIds = [], viewSpecs = []) {
  ...
  const commonLayout = Object.fromEntries(
    Object.entries(figure.layout || {}).filter(
      ([key]) =>
        !/^[xy]axis\d*$/.test(key) &&
        key !== "width" &&
        key !== "height" &&
      ...
      return {
        ...
        figure: {
          ...
          layout: {
            ...
            xaxis: independentAxis(
              figure.layout?.[axisLayoutKey(group.xRef, "x")],
            ),
            yaxis: independentAxis(
              figure.layout?.[axisLayoutKey(group.yRef, "y")],
            ),
          },
