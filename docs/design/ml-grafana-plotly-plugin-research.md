...
| bounded sanitizer | 無。 | PoC 只准 `bar`、`scatter`、`heatmap`；固定 host-owned config；不收 `frames`、`transforms`、`customdata`、`ids`、`meta`、`text/hovertemplate`、`layout.images/template/updatemenus/sliders`、URL、HTML、script、onclick；所有數字 finite。 | 不存在 browser-side compute、external resource、任意 HTML/JS 或 raw-row carrier。 |
